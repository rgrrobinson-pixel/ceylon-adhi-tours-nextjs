...
export const tourPackagesSection = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'array',
      of: [{ type: 'string' }],
      description: 'List the items included with every package.',
    }),
    defineField({
      name: 'exclusions',
      title: 'Not included',
      type: 'array',
      of: [{ type: 'string' }],
      description: 'List the items guests pay for separately.',
    }),
    defineField({
      name: 'footnote',
      title: 'Package pricing note',
      type: 'text',
      rows: 3,
      description: 'Short note shown below the package cards.',
    }),
    defineField({
      name: 'ctaLabel',
      title: 'Package button label',
      type: 'string',
      description: 'Text shown on each package enquiry button.',
      initialValue: 'Ask about a package',
    }),
    defineField({
